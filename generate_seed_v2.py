...
def generate_sql(path):
    ...
    for row in sheet.iter_rows(min_row=7):
        action_item = row[2].value
        if not action_item: continue
        
        if action_item in phase_map:
            phase_name = action_item
        ...
        # Add more complex RAG logic if needed later
...
VALUES ('{}', {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, '{}', {}, {}, {}, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);""".format(
            task_id, action_item_val, priority, status, start_date, due_date, duration, actual_end, actual_days, variance, evidence, action_type, rag, completed_by, owner_id, current_phase, resources, cost, notes
        )
        sql_statements.append(sql)

    with open("seed_v2.sql", "w") as f:
        f.write("\n".join(sql_statements))
